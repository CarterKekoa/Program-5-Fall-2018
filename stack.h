...
public: // operations are called methods or member functions
   ...
   /* for function
   bool isAdded;
   Stack copyStack; //put somewhere to copy stack otherwise when printed the stack will disapear
   ItemType topItem;
   while (not pstack.isEmpty())
   {
     pstack.retrieve(topItem); //get top item
     copyStack.push(topItem, isAdded);
     //TODO move to bottom to print dif cout << topItem << endl; //print the top item
     pstack.pop(); //reduces stack of its top item
   }
   while (not copyStack.isEmpty())
   {
     copyStack.retrieve(topItem);
     pstack.push(topItem, isAdded);
     cout << topItem << endl; //print the top item
     copyStack.pop();
   }
   */
